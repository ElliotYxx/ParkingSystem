...
global SZ, MAX_WIDTH, Min_Area, PROVINCE_START, provinces, cardType, Prefecture, config, model, cn_model
SZ = args.Size  # 训练图片长宽
...
def find_location(src):
    ...
    if pic_width > MAX_WIDTH:
        ...
        img = cv2.resize(img, (MAX_WIDTH, int(pic_height * resize_rate)),
...
def identify_plate(card_imgs, colors, model, cn_model):
    # 识别车牌中的字符
    result = {}
    predict_result = []
    roi = None
    ...
    for i, color in enumerate(colors):
        if color in ("blue", "yellow", "green"):
            ...
            for i, part_card in enumerate(part_cards):
                ...
                if i == 0:  # 识别汉字
                    resp = cn_model.predict(part_card)  # 匹配样本
...
def start_recognize(src):
    result = {}
    start = time.time()
    if os.path.exists("/Users/sheva/IdeaProjects/ParkingSystem/parking-admin/src/main/resources/py/svm.dat"):
        model.load("/Users/sheva/IdeaProjects/ParkingSystem/parking-admin/src/main/resources/py/svm.dat")
    else:
        raise FileNotFoundError("svm.dat")
    if os.path.exists("/Users/sheva/IdeaProjects/ParkingSystem/parking-admin/src/main/resources/py/svmchinese.dat"):
        cn_model.load("/Users/sheva/IdeaProjects/ParkingSystem/parking-admin/src/main/resources/py/svmchinese.dat")
    else:
        raise FileNotFoundError("svmchinese.dat")

    card_imgs, colors = find_location(src)
    if card_imgs is []:
        print('车牌区域提取为空（card_imgs）')
        return
    else:
        predict_result, roi, card_color = identify_plate(card_imgs, colors, model, cn_model)
        if predict_result:
            result['UseTime'] = round((time.time() - start), 2)
            result['InputTime'] = time.strftime("%Y-%m-%d %H:%M:%S")
            result['Type'] = cardType[card_color]
            result['List'] = predict_result
            result['Number'] = ''.join(predict_result[:2]) + '·' + ''.join(predict_result[2:])
            result['Picture'] = roi
            try:
                result['From'] = ''.join(Prefecture[result['List'][0]][result['List'][1]])
            except:
                result['From'] = '未知'
            return result
        else:
...
if __name__ == '__main__':
    result = start_recognize(args.Pic_path)
    print(result['Number'])
    print(result['Type'])
